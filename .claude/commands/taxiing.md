...
3. **plan.md 로딩**: `PLAN_FILE` 을 읽어 구현 단계 / 변경 파일 / 테스트 전략 / 수락 기준을 파악.
4. **구현 진행**:
...
6. **`console.log` 검사**: 변경된 프런트엔드 파일에 `console.log` 가 남아있으면 사용자에게 알린다.
7. **남은 변경 커밋**: `git status` 가 깨끗해질 때까지 커밋. 커밋되지 않은 변경이 남으면 종료하지 않는다.
8. **상태 파일 갱신**: `taxiing.done_at = <UTC ISO8601>` 저장.
9. **요약 출력**: 추가된 커밋 수와 통과된 테스트 종류를 한두 문장으로 안내.
