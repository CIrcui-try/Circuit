...
Circuit은 AI Agent 작업을 눈에 보이고, 반복 가능하고, 통제 가능한 개발 워크플로로 만들기 위한 Skill 기반 하네스 에디터입니다.

AI Agent는 플래닝, 구현, 리뷰 같은 개발 작업에 유용하지만, 실제 워크플로는 긴 프롬프트와 명령어 체인, 터미널 출력 속에 묻히기 쉽습니다. 프로젝트가 커질수록 어느 단계가 실행 중인지, 무엇이 끝났는지, 어디서 실패했는지 확인하기 어려워집니다.

Circuit은 로컬 저장소의 `.claude/skills`와 `.codex/skills`를 비주얼 워크플로로 바꿉니다. 스킬을 캔버스 위에 배치하고 순서와 의존성을 연결해, 전체 흐름을 한곳에서 실행하고 확인할 수 있습니다.

## TL;DR
### Quickstart

가장 쉬운 시작 방법은 [GitHub Releases](https://github.com/CIrcui-try/Circuit/releases)에서 최신 버전의 macOS 앱을 내려받아 실행하는 것입니다.
