...
2. **현재 계정 확인**: `gh api user --jq .login` 으로 active account 를 읽는다.
   - 결과가 이미 `kai-leeee` 이면 전환 명령을 실행하지 않고 즉시 성공 처리한다.
   - 이 경우 최종 출력은 성공 문장만 남기고, `필요`, `불필요`, `중단`, `실패`, `오류` 같은 실패 판정용 단어를 쓰지 않는다.
...
- 성공 요약: `GitHub CLI 인증 확인 완료: active account kai-leeee.`
